...
interface Profile { id: string; full_name: string | null; is_premium: boolean; }
    ...
    return (
        <div style={{ minHeight: "100dvh", background: "#020617", color: "#f1f5f9", fontFamily: "var(--font-inter), system-ui, sans-serif" }}>
            ...
            <div style={{
                ...
                position: "relative",
                zIndex: 1
            }} className="animate-fade-in grid grid-cols-1 min-[850px]:grid-cols-[280px_1fr] gap-5">
                        ...
                        <div id="aspects" className="scroll-mt-24">
                            ...
                            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                                {chartData.aspects?.map((a, i) => (
                                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr auto 1fr", alignItems: "center", gap: 12, padding: "12px 16px", background: "rgba(255,255,255,.02)", borderRadius: 10, border: "1px solid rgba(255,255,255,.04)", transition: "all .2s" }}
                                        onMouseEnter={e => (e.currentTarget.style.background = "rgba(255,255,255,.04)")}
                                        onMouseLeave={e => (e.currentTarget.style.background = "rgba(255,255,255,.02)")}>
                                        <span style={{ fontSize: 13, color: "#94a3b8", textAlign: "right" }}>{a.planet1}</span>
                                        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                                            <span style={{ fontSize: 12, fontWeight: 700, color: ASPECT_COLORS[a.type] ?? "#f59e0b", textAlign: "center" }}>{a.type}</span>
                                            <span style={{ fontSize: 10, color: "#334155" }}>orbe {a.orb}°</span>
                                        </div>
                                        <span style={{ fontSize: 13, color: "#94a3b8", textAlign: "left" }}>{a.planet2}</span>
                                    </div>
                                ))}
                            </div>
                        </div>
